...
async def is_blocked(update, context):
    ...
    try:
        ...
        pass
    try:
        member = await context.bot.get_chat_member(chat_id="@jado_ichancy", user_id=user_id)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        supa.upsert_user(telegram_id=user_id, username=username,
                         first_name=update.effective_user.first_name or "",
                         last_name=update.effective_user.last_name or "")
    except Exception:
        pass

    ALWAYS_ALLOWED = {"check_sub", "agree", "reject", "terms_and_conditions"}
