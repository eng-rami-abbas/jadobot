...
async def handle_create_account(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # OPTIMIZED: Try once immediately, if failed try once more after 1 second
    player_id = None
    if result.get('success'):
        ...
        if not player_id:
            ...
            try:
                ...
                logger.info(f"Immediate verification: player_id={player_id}")
            except Exception as e:
                logger.error(f"Immediate verify error: {e}")
                player_id = None
            
            # If still no player_id, try one more time after 1 second
            if not player_id:
                await asyncio.sleep(1)  # Reduced from 2 seconds
                try:
                    player_id = await api.get_player_id_by_username(name)
                    logger.info(f"Retry verification: player_id={player_id}")
                except Exception as e:
                    logger.error(f"Retry verify error: {e}")
                    player_id = None
