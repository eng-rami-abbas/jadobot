...
async def button_deposit_account_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # OPTIMIZED: Store API instance in context to avoid re-creation
...
async def get_ammount_for_deposit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if deposit_result.get('success'):
        ...
        try:
            ...
            # تسجيل العملية في Supabase - parallel logging
            # Fire and forget for non-critical logs
            asyncio.create_task(_log_deposit_async(
                telegram_id, ammountForDeposit, deposit_result.get('new_balance', 0), username_ichancy
            ))
            ...
            logger.error(f"Error updating balance after deposit: {e}")
            # Still show success since the API call worked
            await update.message.reply_text(
                f"✅ **تم الشحن بنجاح!**\n\n"
                f"👤 الحساب: `{username_ichancy}`\n"
                f"💰 المبلغ: {ammountForDeposit:,.0f} ل.س\n\n"
                f"⚠️ تم الشحن في الموقع لكن حدث خطأ في تحديث رصيد البوت. تواصل مع الدعم.",
                parse_mode="Markdown"
            )
    else:
...
async def _log_deposit_async(telegram_id, amount, new_balance, username_ichancy):
    ...
    try:
        ...
        logger.warning(f"Async deposit logging error: {e}")
